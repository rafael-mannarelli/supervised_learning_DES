% name2 = selfloop(name1, list)
function selfloop(name2, name1, list)
global path;
global tct_name;
global prm_file; 
global rst_file;
...
%--------------------------------------------------------------------------
% generate the prm file for selfloop
...
fprintf(fid, '%s = Selfloop(%s,[', name2, name1);
for i = 1: length(list)
    fprintf(fid, '%d', list(i));
    if i < length(list)
        fprintf(fid, ',');
    end
end
