% supo = supscop(plant, sup, null_list)
function supscop(supo, plant, sup, null_list)
global path;
global tct_name;
global prm_file; 
global rst_file;
...
%--------------------------------------------------------------------------
% generate the prm file for supscop
...
full_name = strcat(path, '\');
full_name = strcat(full_name, supo);
full_name = strcat(full_name, '.DES');
...
[state_size, tran_size] = getdes_parameter(supo);
...
fprintf(fid, '%s = Supscop(%s,%s,Null[', supo, plant, sup);
for i = 1:length(null_list)
