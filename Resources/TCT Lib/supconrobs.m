% supcro = supconrobs(plant, sup, null_list)
function supconrobs(supcro, plant, sup, null_list)
global path;
global tct_name;
global prm_file; 
global rst_file;
...
%--------------------------------------------------------------------------
% generate the prm file for supconrobs
...
full_name = strcat(path, '\');
full_name = strcat(full_name, supcro);
full_name = strcat(full_name, '.DES');
...
[state_size, tran_size] = getdes_parameter(supcro);
...
fprintf(fid, '%s = Supconrobs(%s,%s,Null[', supcro, plant, sup);
for i = 1:length(null_list)
